...
export default function FilterCategory({ onFilterChange, category, status }) {
  ...
  const handleCategoryChange = (item) => {
    ...
    onFilterChange(updatedCategories, selectedStatuses); // Pass both categories and statuses
  };

  const handleStatusChange = (item) => {
    const updatedStatuses = selectedStatuses.includes(item)
      ? selectedStatuses.filter((status) => status !== item)
      : [...selectedStatuses, item];
    setSelectedStatuses(updatedStatuses);
    onFilterChange(selectedCategories, updatedStatuses); // Pass both categories and statuses
  };
  ...
  return (
    <div className="relative z-10">
      ...
      >
        ...
        <h6 className="mt-4 mb-3 text-sm font-medium text-slate-900 dark:text-white">
          Pilih Status
        </h6>
        <ul className="space-y-2 text-sm grid grid-cols-2">
